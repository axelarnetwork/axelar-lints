...
use rustc_hir::{Item, ItemKind, TyKind, QPath, FieldDef};
    ...
    fn check_item(&mut self, cx: &LateContext<'tcx>, item: &'tcx Item<'tcx>) {
        match item.kind {
            ItemKind::Struct(ident, variant_data, _) => {
                if ident.name.as_str() != "ExecuteMsg" && ident.name.as_str() != "InstantiateMsg" && ident.name.as_str() != "QueryMsg" && ident.name.as_str() != "MigrateMsg" {
                    return;
                }
